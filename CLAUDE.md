...
- **yapstack-transcription** — Model management for Whisper (ggml, single file), Parakeet TDT (multi-file ONNX bundle in a per-variant directory), and Sortformer (single ONNX file for diarization), all from HuggingFace with streaming SHA-256 verification. `TranscriptionClient` (formerly `WhisperClient`, now engine-agnostic — alias retained for one release) spawns the sidecar with `--engine` and communicates via JSON-line IPC. `transcribe_with(audio_path, language, initial_prompt, diarization)` exposes the per-call diarization flag.
...
**State management (backend)**: Four `Arc<Mutex<T>>` states in Tauri — `AudioManagerState`, `ModelManagerState`, `WhisperClientState` (now holds a `TranscriptionClient` for either engine; the type alias name is unchanged for backwards-compat), `LiveTranscriptionState`. The `WhisperClientState` wraps `Option<TranscriptionClient>` (None until `init_whisper_client` or `init_transcription_client` is called). The `LiveTranscriptionState` wraps `Option<LiveTranscriptionRuntime>` (controller + dynamic vocabulary hints; None until live transcription starts).

**State management (frontend)**: Zustand store (`appStore.ts`) with persisted settings (**version 22**, with migrations). SQLite via `tauri-plugin-sql` for session/segment/note/folder/tag/chat-message/dictation-history persistence (`lib/db.ts`). Segment writes serialized via a promise queue to prevent backfill/live race conditions. Navigation model: views `"note-list" | "note-detail" | "settings"` with `ListFilter { type: "all" | "pinned" | "folder" | "dictation", folderId? }`. **DB schema at migration v11**: sessions, segments, folders, notes, note_versions, shares, chat_messages, session_folders, dictation_history, **tags, session_tags**. The Parakeet+Sortformer `segments.speaker_id INTEGER` column is *not* in the migration list — it's added by `db::ensure_runtime_schema()` (idempotent ALTER at startup) for compatibility with dev DBs whose `_sqlx_migrations` history predates the merged v11 entry. Settings carry `selectedEngine` (Whisper | Parakeet, peers — Whisper is the upgrade-safe default), `selectedParakeetVariant`, `diarizationEnabled`, and a per-session `speakerNames` map for renaming `Speaker N` → custom labels. Store also tracks `tags: DbTag[]` and `sessionTagMap: Record<string, string[]>` alongside the folder equivalents.
...
**AI chat & tool calling**: `FloatingChatBar` provides per-session AI chat using OpenAI-compatible APIs with multi-turn tool calling. Six tools: `update_title`, `save_to_notes`, `pin_session`, `tag_session`, `add_to_folder`, `get_folder_context`. Multi-turn execution loop sends tool results back to the LLM for phased workflows (classify folder → write summary). Action directives use two-phase tool chaining. Tool execution state rendered by `ToolExecutionBlock` with per-tool status indicators. Adding a new tool only requires a `registerTool()` call in `ai-tools.ts`. See `docs/ARCHITECTURE.md` § "AI Chat & Tool Calling" for full details.
