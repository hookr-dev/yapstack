...
### Added
- Local semantic search across transcripts, dictations, and notes. A new `yapstack-embedding-sidecar` ships alongside the transcription sidecar and runs `BAAI/bge-small-en-v1.5` (~67 MB, 384-dim) locally — every English segment, dictation, and note is embedded in the background and stored in `sqlite-vec` virtual tables. The AI chat exposes a new `search_semantic` tool (English-only, scope-clamped to the active chat's session/folder/dictation view) so the model can find content by meaning when keyword search misses. Defaults on; opt-out lives under Settings → Transcription → Advanced ("Local semantic search").
