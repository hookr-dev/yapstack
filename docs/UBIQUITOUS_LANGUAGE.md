...
| **Device broker**   | The always-on Tauri-side task that owns the receiving end of the audio crate's runtime-agnostic device-event sink. Debounces bursty Core Audio listener events in a 250 ms window, emits `devices-changed` / `default-device-changed` to the FE, and dispatches **Auto-failover** restart intents through the live-transcription loop (or directly to `AudioManager` when no live session is active). | Device manager                    |
