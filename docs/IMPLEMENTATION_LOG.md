...
- **`LiveSegmentEvent` and `LiveTranscriptionPressureEvent` carry `origin: SegmentOrigin`.** Replaces the earlier `is_backfill: bool` outright — the desktop app and its sidecar deploy atomically, so there's no version skew to bridge, and `origin` is a strict superset (distinguishes `live` from `final_flush`, where the boolean could not). `LiveSegmentEvent` also gains `event_sequence: u64` as a stable tie-breaker for same-offset segments across sources.
